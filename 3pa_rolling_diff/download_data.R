...
get_nba_data(seasons = seq(2011, 2021), data = "shotdetail", untar = TRUE)
...
create_season_shotdetail <- function(season="2022-23", team_dict){
  ...
  l <- list()
  for(team_id in seq(1, length(team_dict))){
    url <- paste0('https://stats.nba.com/stats/shotchartdetail?AheadBehind=&CFID=&CFPARAMS=&ClutchTime=&Conference=&ContextFilter=&ContextMeasure=FGA&DateFrom=&DateTo=&Division=&EndPeriod=10&EndRange=28800&GROUP_ID=&GameEventID=&GameID=&GameSegment=&GroupID=&GroupMode=&GroupQuantity=5&LastNGames=0&LeagueID=00&Location=&Month=0&OnOff=&OppPlayerID=&OpponentTeamID=0&Outcome=&PORound=0&Period=0&PlayerID=0&PlayerID1=&PlayerID2=&PlayerID3=&PlayerID4=&PlayerID5=&PlayerPosition=&PointDiff=&Position=&RangeType=0&RookieYear=&Season=',
                  season, '&SeasonSegment=&SeasonType=Regular%20Season&ShotClockRange=&StartPeriod=1&StartRange=0&StarterBench=&TeamID=',
                  team_dict[[team_id]], '&VsConference=&VsDivision=&VsPlayerID1=&VsPlayerID2=&VsPlayerID3=&VsPlayerID4=&VsPlayerID5=&VsTeamID=')
    print(team_dict[[team_id]])
    res <- GET(url, add_headers(nba_request_headers))
    json <- fromJSON(content(res, as = "text"))
    shot_data <- data.table(json$resultSets$rowSet[[1]])
    names(shot_data) <- json$resultSets$headers[[1]]
    l[[team_id]] <- shot_data
    Sys.sleep(5)
  }
  return(rbindlist(l))
...
shot_detail <- create_season_shotdetail(team_dict = team_dict)

fwrite(shot_detail, "shotdetail_2022.csv")
